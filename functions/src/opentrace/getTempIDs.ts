...
const getTempIDs = async (uid: string) => {
  ...
  const tempIDs = await Promise.all(
    [...Array(config.tempID.batchSize).keys()].map(async (i) => generateTempId(encryptionKey, uid, i))
  );
...
export function decryptTempID(
  tempID: string,
  encryptionKey: Buffer
): { uid: string; startTime: number; expiryTime: number } {
